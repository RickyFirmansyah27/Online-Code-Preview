...
export default function JsonTreeSidebar({
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="bg-[#12121a]/90 backdrop-blur rounded-xl border border-white/[0.05] p-6 flex flex-col h-full">
        <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
          <Database className="w-5 h-5 text-blue-400" />
          JSON Files
        </h3>

        <div className="space-y-2 mb-6 flex-1 overflow-y-auto min-h-0 custom-scrollbar pr-2">
          {jsonFiles.map((file) => (
            <motion.button
              key={file.id}
              onClick={() => onFileSelect(file)}
              className={`w-full text-left p-3 rounded-lg transition-colors shrink-0 ${activeFile?.id === file.id
        ...
        {jsonFiles.length === 0 && (
          <div className="text-center py-8 text-gray-400 mb-6 flex-1">
        ...
        <div className="flex flex-col gap-3 pt-4 border-t border-white/[0.05] mt-auto shrink-0">
